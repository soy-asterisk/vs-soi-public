...
class Character extends FunkinSprite implements IBeatReceiver implements IOffsetCompatible
{
	...
	public function fixChar(switchAnims:Bool = false)
	{
		...
		if(danceSpeed<=0) danceSpeed = isDanceLeftDanceRight?1:2;
		if (isPlayer)
			flipX = !flipX;
		__baseFlipped = flipX;
	}
	...
	override function update(elapsed:Float)
	{
		...
		if((lastAnimContext==SING || lastAnimContext==MISS) && !(lastAnimContext==SING && __lockAnimThisFrame))
			tryDance();
	}
	...
	public override function beatHit(curBeat:Int) {
		...
		if (danceOnBeat && (curBeat % (danceInterval*danceSpeed) == 0) && !(lastAnimContext==SING && __lockAnimThisFrame))
		{
			tryDance();
		}
	}
	...
	public function applyXML(xml:Access) {
		...
		if (xml.x.exists("danceSpeed")) danceSpeed = Std.parseInt(xml.x.get("danceSpeed"));
