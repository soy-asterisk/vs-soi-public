...
class PlayState extends MusicBeatState
{
	...
	public var duetCameraSetting:DuetCameraSetting = {enable:false,target:[0,1],ratio:0.5};
	...
	@:dox(hide) override public function create()
	{
		...
		if (smoothTransitionData != null && smoothTransitionData.stage == curStage) {
			...
			var positions = [];
			var len;
			for(line in strumLines) for(c in line.characters) if(c!=null)
				positions.push(c.getCameraPosition());
			if((len = positions.length)>0){
				var pos = FlxPoint.get(0,0);
				for(e in positions){
					pos.x+=e.x;pos.y+=e.y;
				}
				camFollow.setPosition(pos.x/len,pos.y/len);
				FlxG.camera.focusOn(camFollow.getPosition());
				pos.put();
			}
			while(positions.length>0) positions.shift().put();
		}
		...
		botplayTxt = new FunkinText(0, strumLine.y + Note.swagWidth*0.5,FlxG.width,"BOTPLAY",32);
		botplayTxt.y -= botplayTxt.height * 0.5;
		botplayTxt.visible = botplay;

		for(text in [scoreTxt, missesTxt, accuracyTxt, botplayTxt]) {
			if(downscroll && text != botplayTxt){
				text.y = healthBarBG.y - (30-healthBarBG.height+text.height);
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if(botplayTxt.visible = botplay){
			botplayTxtAlphaRot = (botplayTxtAlphaRot + (elapsed * Math.PI * 0.75)) % (Math.PI * 2);
			botplayTxt.alpha = (2 / 3) + (Math.sin(botplayTxtAlphaRot) / 3);
		}
		...
		iconScale = lerp(iconScale,1,0.33);
		{
			var p1Scale:Float = iconScale;
			var p2Scale:Float = iconScale;
			if(curCameraTarget==1) p2Scale=1;
			else if(curCameraTarget==0) p1Scale=1;
			iconP1.scale.set(p1Scale, p1Scale);
			iconP2.scale.set(p2Scale, p2Scale);
		}
		...
		if(downscroll){
			iconP1.y -= (iconP1.height-iconP1.frameHeight);
			iconP2.y -= (iconP2.height-iconP2.frameHeight);
		}
		...
		if(generatedMusic){
			if(duetCameraSetting.enable){
				var targetPos:Array<FlxPoint> = [];
				var chars = 0;
				for(ct in duetCameraSetting.target){
					if(strumLines.members[ct] == null) break;
					var r = 0;
					var pos:FlxPoint;
					pos = FlxPoint.get();
					for(c in strumLines.members[ct].characters) {
				...
				if(targetPos.length==2){
					var ratio = duetCameraSetting.ratio;
					var pos = FlxPoint.get(
						FlxMath.lerp(targetPos[0].x,targetPos[1].x,ratio),
						FlxMath.lerp(targetPos[0].y,targetPos[1].y,ratio)
					);
					var event = scripts.event("onCameraMove", EventManager.get(CamMoveEvent).recycle(pos, [for(ct in duetCameraSetting.target) strumLines.members[ct]], chars, true, ratio));
					if (!event.cancelled)
						camFollow.setPosition(pos.x, pos.y);

					pos.put();
				}
				for(p in targetPos) p.put();
			}
			else if (strumLines.members[curCameraTarget] != null)
			{
				var pos = FlxPoint.get();
				var r = 0;
	...
	public function executeEvent(event:ChartEvent) {
		...
		switch(event.name) {
			...
			case "Duet Camera":
				if (event.params[0] is Bool && event.params[1] is Int && event.params[2] is Int && event.params[3] is Float)
					duetCameraSetting = {
						enable:event.params[0],
						target:[event.params[1],event.params[2]],
						ratio:event.params[3]
					};
			case "Unknown":
		}
	}
	...
	public function endSong():Void
	{
		...
		if (validScore && !botplay)
		{
			#if !switch
			switch(SONG.meta.name){
				case "daybreak":
					FunkinSave.daybreakClear = true;
			}
	...
	public function nextSong() {
		if (isStoryMode)
		{
			...
			if( storyWeek.id == "soi"
				...
				&& storyPlaylist[0].toLowerCase() == "daybreak"){
				//hard以外の場合、Daybreakをプレイすることはできない
				storyPlaylist.resize(0);
			}
	...
	public function displaySongNameCard() {
		//var fieldWidth = 300;
		var fontSize = 32;
		var height = fontSize*3;
		var grp = new FlxSpriteGroup();
		var back = new FunkinSprite(FlxG.width,FlxG.height).makeSolid(FlxG.width,FlxG.height,0xFF000000);
		grp.add(back);
		back.cameras = [camHUD];
		back.alpha = 0.5;
		
		var name = new FunkinText(FlxG.width-fontSize,FlxG.height-height,0,SONG.meta.displayName,fontSize);
		grp.add(name);
		name.color = SONG.meta.color.getColorFromDynamic().getDefault(0xFFFFFFFF);
		if(name.color.red < 0x44 || name.color.green < 0x44 || name.color.blue < 0x44){
			name.color = FlxColor.add(name.color,0x00444444);
		}
		name.borderSize = 2;
		name.x-=name.width;
		name.cameras = [camHUD];
		...
		var artist = new FunkinText(FlxG.width-fontSize+1,FlxG.height-height+fontSize,0,
			SONG.meta.artist==null?"":SONG.meta.artist,Std.int(fontSize*0.6));
		
		grp.add(artist);
		artist.x-=artist.width;
		artist.cameras = [camHUD];
		artist.alignment=RIGHT;
	
		back.x -= Math.max(name.width,artist.width)+fontSize*2;
		back.y = FlxG.height-(name.height+artist.height+fontSize);
		artist.y = FlxG.height-fontSize*0.5-artist.height;
		name.y = artist.y-name.height;
	
		add(grp);
		grp.alpha = 0.00001;//0にするとすべてのアルファが0になってしまう(↓のtweenで元のアルファに戻らない)
	
		if(!downscroll){
			grp.y+=height;
			FlxTween.tween(grp,{alpha:1,y:grp.y-height},0.5,{
				onComplete:function(twn){
					FlxTween.tween(grp,{alpha:0,y:grp.y+height},0.5,{
						onComplete:function(twn){
							remove(grp,true);
							grp.destroy();
						},
			...
			for(obj in [back,name,artist])
				obj.y = camHUD.height - obj.y - obj.height;
			grp.y-=height;
			FlxTween.tween(grp,{alpha:1,y:grp.y+height},0.5,{
				onComplete:function(twn){
					FlxTween.tween(grp,{alpha:0,y:grp.y-height},0.5,{
