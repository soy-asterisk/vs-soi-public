...
class FreeplayState extends MusicBeatState
{
	...
	public var lerpAcc:Float = 0;

	public var intendedAcc:Float = 0;
	...
	override function update(elapsed:Float)
	{
		...
		if(Math.abs(lerpAcc - intendedAcc) <= 0.001)
			lerpAcc = intendedAcc;
		...
		if (canSelect) {
			...
			if(gamepad!=null){
				changeCoopMode((gamepad.justPressed.LEFT_SHOULDER ? -1 : 0) + (gamepad.justPressed.RIGHT_SHOULDER ? 1 : 0));
			}
		...
		scoreText.text = 'PERSONAL BEST:${lerpScore}[${FlxMath.roundDecimal(lerpAcc * 100, 2)}%]';
	...
	public function changeCoopMode(change:Int = 0, force:Bool = false) {
		...
		var event = event("onChangeCoopMode", EventManager.get(MenuChangeEvent).recycle(curCoopMode, FlxMath.wrap(curCoopMode + change, 0, bothEnabled ? 2 : 1), change));
