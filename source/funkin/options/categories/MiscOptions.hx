...
class MiscOptions extends OptionsScreen {
	public override function new() {
		...
		add(new TextOption(
			...
			function() {
				parent.add(Type.createInstance(ResetSaveOption,[]));
		}));
	}
}

class ResetSaveOption extends OptionsScreen {
	public override function new() {
		super("Reset Save Data", "Select this option to reset save data. This will remove all of your highscores");
		add(new TextOption(
			"No",
			"",
			function() {
		...
		add(new TextOption(
			...
			"",
			function() {
				FunkinSave.daybreakClear = false;
				FunkinSave.resetHighscore();
				close();
		}));
	}
}
